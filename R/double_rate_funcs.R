...
#' @description From a set of matched values that represent area/growth readouts where one group is a known double of another, calculate a doubling time. This function was designed to measure spheroid area from IncuCyte images were cells were initially seeded at 1,000 and 2,000 cells separately. It is recommended that data be generated from the `fit_model` function, and then `predict_data` used to generate a large number of points to aid in calculations.
...
calc_double_rate_group <- function(data, x_var = "x", y_var = "y", shared_group = "group", n, two_n, x_lab = "x", y_lab = "y") {
  ...
  # extract data
  data_x <- filter(data, group == n) |> pull(x)
  data_n <- filter(data, group == n) |> pull(y)
  data_two_n <- filter(data, group == two_n) |> pull(y)
  
  # ratio approach
  data_ratio <- log(data_two_n / data_n)
  
  # assume sigmoid growth
  ...
  # plot
  df_overall <- mutate(data, linear = case_when(x %in% data_x[linear_indexes] ~ TRUE, .default = FALSE))
  df_overall <- mutate(df_overall, group = case_when(linear == TRUE ~ "linear", .default = group))
  gg_overall <- ggplot(df_overall, mapping = aes(x, y, colour = group)) + 
    geom_point() + 
    scale_colour_manual(values = c("#FFC20A", "#0C7BDC", "#40B0A6") |> set_names(c(n, two_n, "linear")), breaks = c(n, two_n)) + 
    geom_vline(xintercept = c(min(data_x_filt[linear_indexes]), max(data_x_filt[linear_indexes])), linetype = "dashed") + 
    labs(x = x_lab, y = y_lab) + 
    theme_incur
  
  df_ratio <- data.frame(x = data_x, y = data_ratio)
  df_ratio <- mutate(df_ratio, linear = case_when(x %in% data_x[linear_indexes] ~ TRUE, .default = FALSE))
  gg_ratio <- ggplot(df_ratio, mapping = aes(x, y, colour = linear)) + 
    geom_point() + 
    scale_colour_manual(values = c("#40B0A6", "#E1BE6A") |> set_names(TRUE, FALSE)) + 
    ...
    labs(x = x_lab, y = str2expression(str_c("ln(", two_n, "/", n, ")")))  + 
    theme_incur
