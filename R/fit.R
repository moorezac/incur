...
set_default_opts <- function(
  ...
  curve_opts <- curve_opts %??% list()
  shared_opts <- shared_opts %??% list()
  outlier_opts <- outlier_opts %??% list()
...
fit_curve <- function(
  ...
  is_shared <- !is.na(shared_opts$share_group) && !any(is.na(shared_opts$share_params))
    ...
    if (outlier_opts$rout) {
      ...
      if (!all(params$data[[outlier_column]] == FALSE)) {
        ...
        if (!is.na(fit_filtered$error)) {
          message(paste("Error in fitting model without outliers:", fit_filtered$error, sep = " "))
