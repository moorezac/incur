...
#' @param shared_group For nested fits (shared parameters), a string for the column in `data` that refers to individual groups to share fitted parameters.
...
fit_model <- function(data, x_var, y_var, curve_func, start_func = NULL, start_vals = NULL, huber = FALSE, detect_outliers = FALSE, shared_group = NULL, shared_params = NULL, lower_bounds = NULL, upper_bounds = NULL, return_func = FALSE, ...) {
  ...
  if (!rlang::is_null(shared_group)) {
    if (!shared_group %in% colnames(data)) {
      stop(str_c(shared_group, " not found in colnames of data"))
    }
  }
  if (!rlang::is_null(shared_params)) {
    if (any(!shared_params %in% names(formals(curve_func)))) {
      stop("one or more of provided 'shared_params' not found in provided 'curve_func'")
    }
  }
  ...
  if (huber) {
    fit <- huber(fit, final_arguments, data, curve_func, iter_max = 100, k = 1.345, tol = 1e-6)
  }
