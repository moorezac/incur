---
title: "Export and Segment Data from the IncuCyte"
author: "Zachery Moore"
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Export and Segment Data from the IncuCyte}
...
There are multiple methods to export data from an IncuCyte. `dosefitr` expects data to be in the "Meta Imaging Series TIFF" format. Here we are using IncuCyte 2022B Rev 2. 
...
Option 2 is the choice for large and/or repeated analyses. However, this process can be difficult to setup. There are 2 provided scripts to aid with this. First, the `trackmate_cellpose_segment.py` contains a script that is provided to FIJI/ImageJ that contains all `cellpose` and `TrackMate` parameters, and will need to be modified to contain yours. This can be run like so:
